...
2. Update whichever of these are relevant (never delete history — add or
   ...
   - `gfl2_py/docs/known_issues.txt` — add/update issues. Tag each with
   ...
   - `gfl2_py/reference.txt` §6 open issues summary — keep in sync with
     gfl2_py/docs/known_issues.txt; prefix each line `[MAIN]` or `[EXPLORE]` to match.
   - `gfl2_py/docs/decisions.txt` — new numbered entry per significant design decision.
   - `gfl2_py/docs/takeaways.txt` — new numbered entry per generalizable lesson.
   - `gfl2_py/docs/action_items.txt` — mark items DONE, or file new follow-ups.
   - `gfl2_py/docs/technical_design.txt` — new modules or changed responsibilities.
3. Run `python gfl2_py/compile_gfl2.py` then `pytest gfl2_py/tests/ -q --no-header --tb=short`.
