...
`gfl2_py/docs/decisions.txt` sat uncommitted and got swept into a later commit with
no one having deliberately reviewed it as its own thing).

This skill NEVER edits `gfl2_py/docs/known_issues.txt`, `gfl2_py/docs/decisions.txt`,
`gfl2_py/docs/takeaways.txt`, `gfl2_py/docs/action_items.txt`, `gfl2_py/docs/technical_design.txt`,
or `gfl2_py/reference.txt`. It writes everything to one new, gitignored, disposable
...
1. Infer what happened this session: takeaways (generalizable lessons),
   ...
   per gfl2_py/docs/known_issues.txt's own key), and any design decisions made — same
   inference `/land` already does, just not written to the same place.

2. Create `gfl2_py/docs/drafts/` if it doesn't exist, then write ONE file:
   `gfl2_py/docs/drafts/<UTC timestamp, e.g. 20260710-0915>_draft.md` (get the
   ...
   ## Proposed gfl2_py/docs/known_issues.txt entries
   <full entry text, ready to paste in, each tagged with its SCOPE key —
    or "none" if nothing rises to that bar>

   ## Proposed gfl2_py/docs/decisions.txt entries
   <full entry text, or "none">

   ## Proposed gfl2_py/docs/takeaways.txt entries
   <full entry text, or "none">

   ## Proposed gfl2_py/docs/action_items.txt changes
...
3. Run `python gfl2_py/compile_gfl2.py` then `pytest gfl2_py/tests/ -q --no-header --tb=no`
...
4. Print the draft's file path and a one-line summary. Do NOT run
   ...
   find yourself about to Edit `gfl2_py/docs/known_issues.txt` etc., stop, that
