# -*- coding: utf-8 -*-
"""
stat_ocr_v0_1_0.py — Blob + projection/Hu moment OCR for Daily Gunsmoke stat cells.
...
Build templates:
    python stat_ocr_v0_1_0.py --build [--images <glob>] [--font <name>] [--save-crops]

Verify pipeline against Tesseract ground truth:
    python stat_ocr_v0_1_0.py --verify [--images <glob>]
"""
...
def _tmpl_variant_paths(variant: str) -> tuple[Path, Path]:
    ...
    mismatched pct/val template pairing (see gfl2/stat_ocr_v0_1_1.py's
...
class StatOcrV0_1_0:
    ...
    @classmethod
    def load(cls, tmpl_variant: str | None = None) -> "StatOcrV0_1_0":
        ...
        depends on. Matches the mechanism gfl2/stat_ocr_v0_1_1.py already used.
        """
        pct_path, val_path = (
        ...
        for p in (pct_path, val_path):
            if not p.exists():
                raise FileNotFoundError(
                    ...
                    "Run: python -m gfl2.stat_ocr_v0_1_0 --build"
                )
...
def build_templates(
    ...
    stat_val.py — gfl2/stat_ocr_v0_1_0.py's --build CLI and assets/builders/build.py,
...
def verify(
    ...
    if verbose:
        ...
        print(f"StatOcrV0_1_0 verify  ({len(image_paths)} images, {len(samples)} cells)")
...
def _main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="StatOcrV0_1_0 template builder / verifier"
    )
