# -*- coding: utf-8 -*-
"""
gfl2/stat_ocr_v0_1_1.py — aspect-preserving-pad variant of gfl2/stat_ocr_v0_1_0.py.

Uses aspect-preserving (pad-don't-stretch) glyph normalization instead of
v0_1_0's direct-stretch resize, addressing the within-class feature
variation documented in docs/known_issues.txt §15: the v0_1_0 pipeline's
...
This file is a DELIBERATE full copy of gfl2/stat_ocr_v0_1_0.py, not an import-and-wrap.
...
normalization happens here and can NEVER regress the v0_1_0 module or its
test baseline. There is intentionally no shared base class and no merge planned
between this file and gfl2/stat_ocr_v0_1_0.py. Only _extract_pct_glyphs /
...
Benchmarked at accuracy/speed parity with v0_1_0 (debugs/stat_ocr_bench.py);
promoted from debugs/ to gfl2/ so it can be selected at runtime via
`main.py --stat-ocr-engine v0_1_1` (v0_1_0 remains the default).

Own template output files (assets/fonts/stat_pct_padded.py / stat_val_padded.py) —
never touches the v0_1_0 stat_pct.py / stat_val.py.

Build templates:
    python -m gfl2.stat_ocr_v0_1_1 --build [--images <glob>]

Verify pipeline against Tesseract ground truth:
    python -m gfl2.stat_ocr_v0_1_1 --verify [--images <glob>]
"""
...
# gfl2/stat_ocr_v0_1_0.py's own fix (decision 47's full-duplication policy; see that module for
...
# gfl2/stat_ocr_v0_1_0.py, see that module for the full rationale.
...
# ─────────────────────────────────────────────────────────────────────────────
# Feature helpers  (copied from gfl2/stat_ocr_v0_1_0.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def _count_inner_blobs(norm: np.ndarray, min_area: int = 4) -> int:
    """Count enclosed holes inside a normalised glyph image (see gfl2/stat_ocr_v0_1_0.py)."""
...
def _bottom_row_width_frac(norm: np.ndarray) -> float:
    ...
    NORM_W (see gfl2/stat_ocr_v0_1_0.py for the full rationale)."""
    w = norm.shape[1]
...
# ─────────────────────────────────────────────────────────────────────────────
# Blob detection & line splitting (copied unchanged from gfl2/stat_ocr_v0_1_0.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def _find_percent_x_start(blobs: list[tuple]) -> Optional[int]:
    """Detect the x-start of the '%' glyph cluster (see gfl2/stat_ocr_v0_1_0.py for
    ...
    gap. Ported from gfl2/stat_ocr_v0_1_0.py's own fix (known_issues.txt's
    ...
    Also ports gfl2/stat_ocr_v0_1_0.py's PCT_ANCHOR_MIN_AREA noise-speck filter
...
def _normalize_glyph(crop: np.ndarray, norm_w: int, norm_h: int) -> np.ndarray:
    ...
    This is the fix under exploration for known_issues.txt §15: v0_1_0's
...
# ─────────────────────────────────────────────────────────────────────────────
# Classifier (copied unchanged from gfl2/stat_ocr_v0_1_0.py — re-tune HERE if the
...
def _classify(norm: np.ndarray, templates: dict, v_primary: bool = False,
    ...
    See gfl2/stat_ocr_v0_1_0.py:_classify for full documentation of each phase.
    """
    if not templates:
        return '?'
...
def _reconstruct_val(
    ...
    """Reconstruct the val string. Rightmost '?' → 'K'/'M' suffix. See gfl2/stat_ocr_v0_1_0.py."""
...
def _reconstruct_pct(
    ...
    """Reconstruct the pct value string from pct-strip glyphs. See gfl2/stat_ocr_v0_1_0.py."""
    items = [(x, norm, hint) for x, norm, hint in glyphs if hint != 'skip']
    parts = []
    for i, (x, norm, hint) in enumerate(items):
        if hint == '.':
...
def _tmpl_variant_paths(variant: str) -> tuple[Path, Path]:
    ...
    mismatched pct/val template pairing (see gfl2/stat_ocr_v0_1_0.py's identical
    copy of this helper)."""
...
class StatOcrV0_1_1:
    ...
    @classmethod
    def load(cls, tmpl_variant: str | None = None) -> "StatOcrV0_1_1":
        ...
        for p in (pct_path, val_path):
            if not p.exists():
                raise FileNotFoundError(
                    ...
                    "Run: python -m gfl2.stat_ocr_v0_1_1 --build"
                )
    ...
    def read(
        ...
        timer: optional TimerStack — when provided, sub-spans are recorded
          under the caller's active span, mirroring gfl2/stat_ocr_v0_1_0.py:
...
def build_templates(
    ...
    See gfl2/stat_ocr_v0_1_0.py:build_templates for the full contract, including why
...
# ─────────────────────────────────────────────────────────────────────────────
# Collect training / verification data — reuses v0_1_0's cell extraction
...
def _collect_cells(
    ...
    """Extract every stat cell from a list of images. See gfl2/stat_ocr_v0_1_0.py:_collect_cells.

    gt_cache: optional {source: {"pct","val"}} dict (see
    gfl2.stat_ocr_v0_1_0._load_tess_gt_cache) consulted before calling Tesseract
    ...
    gfl2/stat_ocr_v0_1_0.py's _collect_cells, so the same cache file works for both.

    excluded_cells: optional {source: reason} dict (see
    gfl2.stat_ocr_v0_1_0._load_excluded_cells) of known-corrupted-input cells to
    drop entirely. None (the default) auto-loads stat_excluded_cells.json
    from the project root -- this is a full duplicate of gfl2/stat_ocr_v0_1_0.py
...
def verify(
    ...
    """Compare padded-normalize pipeline against Tesseract on every cell. See gfl2/stat_ocr_v0_1_0.py:verify.

    gt_cache: explicit None auto-loads tests/inputs/daily/tess_gt_cache.py
      (debugs/build_tess_gt_cache.py) via gfl2.stat_ocr_v0_1_0._load_tess_gt_cache;
    ...
    if verbose:
        ...
        print(f"\n{'-'*60}")
        print(f"StatOcrV0_1_1 verify  ({len(image_paths)} images, {len(samples)} cells)")
...
def _main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="StatOcrV0_1_1 (aspect-preserving-pad variant, docs/known_issues.txt §15) template builder / verifier"
    )
    ...
    if args.build:
        ...
        # prints the "Applied N" line — see its docstring / gfl2/stat_ocr_v0_1_0.py's
